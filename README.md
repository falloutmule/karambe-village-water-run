...
Open **[index.html](index.html)** directly in a browser, or download it for offline
play. The complete game, SFHS touch runtime, artwork, effects and original arcade
music are contained in that one file. Android Chrome is the primary acceptance
device; automated Chromium results are engineering evidence only.
...
The first release run is sequential. Finishing all three levels unlocks Level
Select on the device. Sound preference and best times persist when browser
storage is available. Private browsing or file-origin storage policies may
limit persistence; downloaded gameplay itself has no network dependency.
...
On localhost or a downloaded file, append `?dev=1` for direct level access and the
`CR.game`/`CR.controls` test hooks. Development sessions do not write progression
or best times. That flag has no effect on public GitHub Pages. Normal builds
