...
for (const phoneTest of [false,true]) {
const artifact=phoneTest?manifest.phoneTestArtifact:manifest.artifact;
const buildId=phoneTest?manifest.phoneTestBuildId:manifest.buildId;
if(!artifact||!buildId)throw Error('Missing build identity or artifact');
const bundled = await build({stdin:{contents: "import { createGameControls } from './controls.ts';\n" + game,resolveDir:process.cwd()+'/src',loader:'js'},define:{__PHONE_TEST_BUILD__:String(phoneTest),__BUILD_ID__:JSON.stringify(buildId)},bundle:true,write:false,format:'iife',target:'es2020',legalComments:'inline',charset:'utf8'});
